...
import { restaurantImages } from "../PortfolioImages";
import Lightbox from "react-image-lightbox";
import "react-image-lightbox/style.css";
import { motion } from "framer-motion";
import WhatsAppButton from "../Components/Whatsapp";

const words = ["Our", "Restaurant & Pub", "Interiors", "Gallery"];
...
const RestaurantInterior = () => {
  ...
  useEffect(() => {
    ...
    const img = new Image();
    img.src = restaurantImages[0].url;
  }, []);
    ...
    <section>
      ...
      <section className="container max-w-6xl mt-12 mx-auto px-2 md:px-4 lg:px-8 py-8  md:py-12 lg:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:my-16 my-8 gap-3 md:gap-8">
          {restaurantImages.map((project, index) => (
            <motion.div
              key={index}
      ...
      {lightbox.isOpen && (
        <Lightbox
          mainSrc={restaurantImages[lightbox.photoIndex].url}
          nextSrc={
            restaurantImages[
              (lightbox.photoIndex + 1) % restaurantImages.length
            ].url
          }
          prevSrc={
            restaurantImages[
              (lightbox.photoIndex + restaurantImages.length - 1) %
          ...
          onMoveNextRequest={() =>
            setLightbox({
              ...lightbox,
              photoIndex: (lightbox.photoIndex + 1) % restaurantImages.length,
            })
          }
        />
      )}
